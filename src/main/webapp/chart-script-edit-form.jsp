...
<c:set var="bodyContent">
        ...
        <form action="ChartScriptEdit" method="POST" class="form-horizontal">
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<select multiple="multiple" class="form-control" id="<%= ModelDomainNames.CHARTSCRIPT_STATDEF.getName() %>" name="<%= ModelDomainNames.CHARTSCRIPT_STATDEF.getName() %>">
					<%
			...
			<div class="form-group">
				<label class="col-sm-2 control-label" for="layerType" title="">Script path</label>
				<div class="col-sm-10">
					<input id="<%= ModelDomainNames.CHARTSCRIPT_SCRIPTPATH.getName() %>" class="form-control" name="<%= ModelDomainNames.CHARTSCRIPT_SCRIPTPATH.getName() %>" type="text" value="<%= res == null ? "" : new ResourceDecorator(res).getAttributeValues(ModelDomainNames.CHARTSCRIPT_SCRIPTPATH).get(0)  %>">
				</div>
			</div>
